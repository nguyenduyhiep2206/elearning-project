...
const createPaymentUrl = async (req, res, next) => {
  try {
    ...
    // Tạo order với status Pending (promotionCode đã được xử lý trong createOrderFromCart)
    ...
    // OrderInfo phải không có ký tự đặc biệt, chỉ chữ và số
...
const vnpayIpn = async (req, res, next) => {
  try {
    ...
    // So sánh với sai số nhỏ để tránh lỗi do làm tròn số thực
    if (Math.abs(amount - order.totalamount) > 0.01) {
      console.error(`Amount mismatch: VNPay=${amount}, Order=${order.totalamount}`);
